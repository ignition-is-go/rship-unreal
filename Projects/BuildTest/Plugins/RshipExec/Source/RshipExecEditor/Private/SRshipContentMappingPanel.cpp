...
namespace
{
	...
	void EnsureContextDefaultsForMapping(URshipContentMappingManager* Manager, const FString& ContextId)
	{
		...
		for (const FRshipRenderContextState& ExistingContext : ExistingContexts)
		{
			...
			{
				UpdatedContext.SourceType = TEXT("camera");
				bNeedsUpdate = true;
			}
...
bool SRshipContentMappingPanel::TryApplySelectionToTarget(TSharedPtr<SEditableTextBox> TargetInput, bool bAppend)
{
		...
		if (URshipSubsystem* Subsystem = GEngine->GetEngineSubsystem<URshipSubsystem>())
		{
			if (URshipContentMappingManager* Manager = Subsystem->GetContentMappingManager())
			{
				...
				for (const FRshipMappingSurfaceState& Surface : KnownSurfaces)
				{
					...
					{
						KnownSurfaceIds.Add(Surface.Id);
					}
				}
			}
		}
	...
	for (FSelectionIterator It(*Selection); It; ++It)
	{
		...
		if (bScreenMode)
		{
			...
			{
				const FString TargetId = ResolveTargetIdForActor(Actor);
				if (!TargetId.IsEmpty())
				{
					const FString CandidateScreenId = ResolveScreenIdInput(TargetId);
					if (KnownSurfaceIds.Contains(CandidateScreenId))
					{
						ResolvedId = CandidateScreenId;
					}
					else
					{
						for (const FRshipMappingSurfaceState& Surface : KnownSurfaces)
						{
							if (Surface.TargetId.Equals(TargetId, ESearchCase::IgnoreCase))
							{
...
int32 SRshipContentMappingPanel::CreateScreensFromSelectedActors()
{
	...
	const TArray<FRshipMappingSurfaceState> ExistingSurfaces = Manager->GetMappingSurfaces();
	int32 CreatedCount = 0;
	bool bSelectionChanged = false;

	auto FindExistingSurfaceIdByMeshName = [&ExistingSurfaces](const FString& MeshName) -> FString
	{
	...
	for (FSelectionIterator It(*Selection); It; ++It)
	{
		...
		const FString ExistingSurfaceId = FindExistingSurfaceIdByMeshName(MeshName);
		...
		NewSurface.Name = (MeshComponents.Num() > 1)
			...
			: BaseName;
		NewSurface.ProjectId = SurfProjectInput.IsValid() ? SurfProjectInput->GetText().ToString().TrimStartAndEnd() : TEXT("");
		NewSurface.TargetId = BaseName;
			...
			if (SourceId.IsEmpty())
			{
				...
				if (ScreenInput.IsEmpty())
				{
					...
					for (FSelectionIterator It(*Selection); It; ++It)
					{
						...
						{
							const FString SelectedTargetId = ResolveTargetIdForActor(Actor);
							if (!SelectedTargetId.IsEmpty())
							{
								const FString Candidate = ResolveScreenIdInput(SelectedTargetId);
								if (SurfacesById.Contains(Candidate))
								{
		...
		else
		{
			...
			if (!ScreenTargetId.IsEmpty())
			{
				...
				for (const FRshipMappingSurfaceState* Candidate : TargetMatches)
				{
					...
					if (!ProjectId.IsEmpty() && Candidate->ProjectId != ProjectId)
					{
						continue;
					}
...
void SRshipContentMappingPanel::RefreshStatus()
{
	...
	if (SurfaceList.IsValid() && bRebuildLists)
	{
		...
		{
			TMap<FString, FString> SurfaceTokenToId;
			...
			for (const FRshipMappingSurfaceState& Surface : SortedSurfaces)
			{
				...
				if (!Surface.Id.IsEmpty())
				{
					SurfaceTokenToId.Add(Surface.Id.TrimStartAndEnd().ToLower(), Surface.Id);
				}
				if (!Surface.Name.IsEmpty())
			{
				SurfaceTokenToId.Add(Surface.Name.TrimStartAndEnd().ToLower(), Surface.Id);
			}
				if (!Surface.MeshComponentName.IsEmpty())
				{
					SurfaceTokenToId.Add(Surface.MeshComponentName.TrimStartAndEnd().ToLower(), Surface.Id);
				}
				SurfaceTokenToId.Add(SurfaceLabel.TrimStartAndEnd().ToLower(), Surface.Id);
			}
					...
					for (const FRshipContentMappingState& Mapping : VisibleMappings)
					{
						...
						const FString ResolutionText = [&]()
						{
							...
							for (const FRshipRenderContextState& Context : SortedContexts)
							{
								...
								{
									return FString::Printf(TEXT("%dx%d"), Context.Width, Context.Height);
								}
								return FString();
							}
						...
						const FString SurfaceIdText = [&]()
						{
							...
							Labels.Reserve(Mapping.SurfaceIds.Num());
							for (const FString& SurfaceId : Mapping.SurfaceIds)
							{
								if (const FString* Label = SurfaceIdToLabel.Find(SurfaceId))
								{
									Labels.Add(*Label);
								}
								else
								{
									Labels.Add(TEXT("(Screen missing)"));
								}
							}
						...
						[
							...
							[
								...
								[
									...
									[
										...
										[
											...
											.SelectAllTextWhenFocused(true)
											.OnTextCommitted_Lambda([this, Mapping, ContextTokenToId, CameraTokenToId, CameraIdToLabel](const FText& NewText, ETextCommit::Type CommitType)
											{
												if (CommitType == ETextCommit::Default || !GEngine)
												{
													return;
										...
										[
											SNew(SComboButton)
											.ContentPadding(CompactMappingButtonPadding)
											.OnGetMenuContent_Lambda([this, Mapping, CameraIdToLabel]()
											{
												FMenuBuilder MenuBuilder(true, nullptr);
												MenuBuilder.AddMenuEntry(
												...
												if (ContextOptions.Num() == 0 && CameraOptions.Num() == 0)
												{
													MenuBuilder.AddWidget(
														SNew(STextBlock).Text(LOCTEXT("MapRowNoContextsOrCameras", "No inputs or cameras found")),
														FText::GetEmpty(),
														true);
												}
									...
									[
										...
										.Font(MappingListFont)
										.HintText(LOCTEXT("MapRowResolutionHint", "WxH"))
										.SelectAllTextWhenFocused(true)
										.OnTextCommitted_Lambda([this, Mapping](const FText& NewText, ETextCommit::Type CommitType)
										{
											if (CommitType == ETextCommit::Default || !GEngine || Mapping.ContextId.IsEmpty())
											{
												return;
											}
											...
											FString ResolutionToken = NewText.ToString().TrimStartAndEnd();
											ResolutionToken.ReplaceInline(TEXT(" "), TEXT(""));

											int32 SepIndex = INDEX_NONE;
											if (!ResolutionToken.FindChar(TEXT('x'), SepIndex) && !ResolutionToken.FindChar(TEXT('X'), SepIndex))
											{
												return;
											}

											const FString WidthToken = ResolutionToken.Left(SepIndex);
											const FString HeightToken = ResolutionToken.Mid(SepIndex + 1);
											const int32 NewWidth = FCString::Atoi(*WidthToken);
											const int32 NewHeight = FCString::Atoi(*HeightToken);
											...
											for (const FRshipRenderContextState& Context : ExistingContexts)
											{
												...
												UpdatedContext.Width = NewWidth;
												UpdatedContext.Height = NewHeight;
												if (UpdatedContext.CaptureMode.IsEmpty())
												{
													UpdatedContext.CaptureMode = TEXT("FinalColorLDR");
												}
									...
									[
										...
										[
											SNew(SEditableTextBox)
											.Text(FText::FromString(SurfaceIdText))
											.Font(MappingListFont)
											.HintText(LOCTEXT("MapRowScreensHint", "screens, comma-separated"))
											.SelectAllTextWhenFocused(true)
											.OnTextCommitted_Lambda([this, Mapping, SurfaceTokenToId](const FText& NewText, ETextCommit::Type CommitType)
											{
												if (CommitType == ETextCommit::Default || !GEngine)
												{
													return;
												...
												TArray<FString> Parts;
												NewText.ToString().ParseIntoArray(Parts, TEXT(","), true);
												for (FString Part : Parts)
												{
													Part = Part.TrimStartAndEnd();
													if (Part.IsEmpty())
													...
													const FString LookupKey = Part.ToLower();
													if (const FString* ResolvedId = SurfaceTokenToId.Find(LookupKey))
													{
														Updated.SurfaceIds.AddUnique(*ResolvedId);
													}
													else
													{
														Updated.SurfaceIds.AddUnique(Part);
													}
												}
										...
										[
											...
											.OnGetMenuContent_Lambda([this, Mapping]()
											{
												...
												else
												{
													for (const TSharedPtr<FRshipIdOption>& Option : SurfaceOptions)
													{
														...
														MenuBuilder.AddMenuEntry(
															...
															FSlateIcon(),
															FUIAction(
																FExecuteAction::CreateLambda([this, Mapping, SurfaceId]()
																{
																	if (!GEngine) return;
																	URshipSubsystem* Subsystem = GEngine->GetEngineSubsystem<URshipSubsystem>();
																	if (!Subsystem) return;
																	...
																	FRshipContentMappingState Updated = Mapping;
																	if (Updated.SurfaceIds.Contains(SurfaceId))
																	{
																		Updated.SurfaceIds.Remove(SurfaceId);
																	}
																	else
																	{
																		Updated.SurfaceIds.Add(SurfaceId);
																	}
																	Manager->UpdateMapping(Updated);
																	Manager->Tick(0.0f);
																	RefreshStatus();
																...
																FCanExecuteAction(),
																FIsActionChecked::CreateLambda([Mapping, SurfaceId]()
																{
																	return Mapping.SurfaceIds.Contains(SurfaceId);
																})),
															NAME_None,
												...
												MenuBuilder.AddSeparator();
												MenuBuilder.AddMenuEntry(
													LOCTEXT("MapRowClearScreens", "Clear Screens"),
													FText::GetEmpty(),
													FSlateIcon(),
													FUIAction(FExecuteAction::CreateLambda([this, Mapping]()
													{
											...
											[
												SNew(STextBlock)
												.Text(LOCTEXT("MapRowScreensPick", "Pick"))
												.Font(CompactMappingButtonFont)
											]
										]
									]
								...
								[
									...
									.OnValueCommitted_Lambda([this, Mapping](float NewValue, ETextCommit::Type CommitType)
									{
										if (CommitType == ETextCommit::Default || !GEngine)
										{
											return;
										...
										FRshipContentMappingState Updated = Mapping;
										Updated.Opacity = FMath::Clamp(NewValue, 0.0f, 1.0f);
										Manager->UpdateMapping(Updated);
										Manager->Tick(0.0f);
										RefreshStatus();
									})
									...
									.OnCheckStateChanged_Lambda([this, Mapping](ECheckBoxState NewState)
									{
										if (!GEngine)
										{
											return;
									...
									[
										...
										[
											...
											{
												SetSelectedMappingId(Mapping.Id);
												if (!SelectedMappingRows.Contains(Mapping.Id))
												{
													SelectedMappingRows.Empty();
													SelectedMappingRows.Add(Mapping.Id);
												}
												OpenMappingEditorWindow(Mapping);
												return FReply::Handled();
											})
										]
										+ SHorizontalBox::Slot().AutoWidth()
										[
											...
											.OnClicked_Lambda([this, Mapping]()
											{
												...
												if (Manager->DeleteMapping(Mapping.Id))
												{
													Manager->Tick(0.0f);
													...
													SelectedMappingRows.Remove(Mapping.Id);
