...
void ARshipCameraActor::RefreshCameraData()
{
    ...
    if (!CameraManager && Subsystem)
    {
        CameraManager = Subsystem->GetCameraManager();
        if (CameraManager && !CameraUpdateHandle.IsValid() && !ColorProfileUpdateHandle.IsValid())
        {
            BindToManager();
        }
    }
    ...
    if (CameraManager->GetCameraById(CameraId, CachedCameraInfo))
    {
        ...
        if (bSyncTransformFromCalibration)
        {
            ...
            {
                SetActorLocationAndRotation(CachedCameraInfo.Position * PositionScale, CachedCameraInfo.Rotation);
            }
        }
