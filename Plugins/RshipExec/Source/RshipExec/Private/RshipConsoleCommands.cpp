    ...
    FConsoleCommandDelegate::CreateLambda([]()
    {
        ...
        if (Result.ErrorCount > 0 || Result.CriticalCount > 0)
        {
            ...
            for (const FRshipValidationIssue& Issue : Result.Issues)
            {
                ...
                {
                    FString ActorName = Issue.AffectedActor ? FString(Issue.AffectedActor->GetActorNameOrLabel()) : TEXT("Unknown");
    ...
    FConsoleCommandDelegate::CreateLambda([]()
    {
        ...
        for (URshipNiagaraBinding* B : Bindings)
        {
            if (B)
            {
                ...
                FString OwnerName = Owner ? FString(Owner->GetActorNameOrLabel()) : TEXT("Unknown");
