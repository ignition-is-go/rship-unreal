...
{
#if WITH_EDITOR
    UMaterial* Mat = NewObject<UMaterial>(GetTransientPackage(), TEXT("RshipContentMappingFallback"));
    ...
    Mat->Expressions.Add(TexParam);

    // Scalar parameters
    UMaterialExpressionScalarParameter* MappingModeParam = NewObject<UMaterialExpressionScalarParameter>(Mat);
    MappingModeParam->ParameterName = ParamMappingMode;
    MappingModeParam->DefaultValue = 0.f;
    Mat->Expressions.Add(MappingModeParam);

    UMaterialExpressionScalarParameter* UVChannelParam = NewObject<UMaterialExpressionScalarParameter>(Mat);
    UVChannelParam->ParameterName = ParamUVChannel;
    UVChannelParam->DefaultValue = 0.f;
    Mat->Expressions.Add(UVChannelParam);

    UMaterialExpressionScalarParameter* UVRotParam = NewObject<UMaterialExpressionScalarParameter>(Mat);
    UVRotParam->ParameterName = ParamUVRotation;
    UVRotParam->DefaultValue = 0.f;
    Mat->Expressions.Add(UVRotParam);
    ...
    OpacityParam->DefaultValue = 1.f;
    Mat->Expressions.Add(OpacityParam);

    UMaterialExpressionVectorParameter* PreviewTintParam = NewObject<UMaterialExpressionVectorParameter>(Mat);
    PreviewTintParam->ParameterName = ParamPreviewTint;
    PreviewTintParam->DefaultValue = FLinearColor::White;
    Mat->Expressions.Add(PreviewTintParam);

    // Vector parameters
    UMaterialExpressionVectorParameter* UVTransformParam = NewObject<UMaterialExpressionVectorParameter>(Mat);
    UVTransformParam->ParameterName = ParamUVTransform;
    UVTransformParam->DefaultValue = FLinearColor(1, 1, 0, 0);
    Mat->Expressions.Add(UVTransformParam);

    UMaterialExpressionVectorParameter* Row0 = NewObject<UMaterialExpressionVectorParameter>(Mat);
    Row0->ParameterName = ParamProjectorRow0;
    Row0->DefaultValue = FLinearColor(1, 0, 0, 0);
    Mat->Expressions.Add(Row0);
    UMaterialExpressionVectorParameter* Row1 = NewObject<UMaterialExpressionVectorParameter>(Mat);
    Row1->ParameterName = ParamProjectorRow1;
    Row1->DefaultValue = FLinearColor(0, 1, 0, 0);
    Mat->Expressions.Add(Row1);
    UMaterialExpressionVectorParameter* Row2 = NewObject<UMaterialExpressionVectorParameter>(Mat);
    Row2->ParameterName = ParamProjectorRow2;
    Row2->DefaultValue = FLinearColor(0, 0, 1, 0);
    Mat->Expressions.Add(Row2);
    UMaterialExpressionVectorParameter* Row3 = NewObject<UMaterialExpressionVectorParameter>(Mat);
    Row3->ParameterName = ParamProjectorRow3;
    Row3->DefaultValue = FLinearColor(0, 0, 0, 1);
    Mat->Expressions.Add(Row3);

    // Custom expression that does UV transform or projection (perspective)
    UMaterialExpressionCustom* Custom = NewObject<UMaterialExpressionCustom>(Mat);
    Custom->Description = TEXT("ContentMapping");
    ...
    Custom->Code = TEXT(R"(
        ...
        {
            float4 clip = float4(
                dot(float4(WorldPos,1), Row0),
                dot(float4(WorldPos,1), Row1),
                dot(float4(WorldPos,1), Row2),
                dot(float4(WorldPos,1), Row3));
            uv = clip.xy / max(clip.w, 1e-3);
            uv = uv * 0.5 + 0.5;
        }
        else
        {
            float2 baseUV = BaseUV;
            float rad = UVRotDeg * 0.01745329252;
            float cosr = cos(rad);
            float sinr = sin(rad);
            float2 scaled = baseUV * UVTransform.xy + UVTransform.zw;
    ...
    auto AddInput = [&](const FString& Name, UMaterialExpression* Expression, ECustomMaterialInputType Type = CMIT_Float1)
    {
        FCustomInput& In = Custom->Inputs.AddDefaulted_GetRef();
        In.InputName = Name;
        In.Input.Expression = Expression;
        In.InputType = Type;
    };

    AddInput(TEXT("Tex"), TexParam, CMIT_Texture2D);
    AddInput(TEXT("MappingMode"), MappingModeParam);
    AddInput(TEXT("UVChannel"), UVChannelParam);
    AddInput(TEXT("UVTransform"), UVTransformParam, CMIT_Float4);
    AddInput(TEXT("UVRotDeg"), UVRotParam);
    AddInput(TEXT("Row0"), Row0, CMIT_Float4);
    AddInput(TEXT("Row1"), Row1, CMIT_Float4);
    AddInput(TEXT("Row2"), Row2, CMIT_Float4);
    AddInput(TEXT("Row3"), Row3, CMIT_Float4);
    ...
    Mat->Expressions.Add(TexCoord);
    AddInput(TEXT("BaseUV"), TexCoord, CMIT_Float2);
    UMaterialExpressionWorldPosition* WorldPos = NewObject<UMaterialExpressionWorldPosition>(Mat);
    Mat->Expressions.Add(WorldPos);
    AddInput(TEXT("WorldPos"), WorldPos, CMIT_Float3);

    Mat->Expressions.Add(Custom);

    // Multiply by opacity
    UMaterialExpressionMultiply* MulOpacity = NewObject<UMaterialExpressionMultiply>(Mat);
    MulOpacity->A.Expression = Custom;
    MulOpacity->B.Expression = OpacityParam;
    Mat->Expressions.Add(MulOpacity);

    UMaterialExpressionMultiply* MulTint = NewObject<UMaterialExpressionMultiply>(Mat);
    MulTint->A.Expression = MulOpacity;
    MulTint->B.Expression = PreviewTintParam;
    Mat->Expressions.Add(MulTint);

    Mat->EmissiveColor.Expression = MulTint;
    Mat->Opacity.Expression = OpacityParam;
...
#else
    UE_LOG(LogRshipExec, Warning, TEXT("ContentMapping material missing; fallback material generation is editor-only."));
#endif
}
