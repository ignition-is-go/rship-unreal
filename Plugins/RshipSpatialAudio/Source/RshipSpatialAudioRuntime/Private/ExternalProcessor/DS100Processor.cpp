...
bool FDS100Processor::SendOSCMessage(const FOSCMessage& Message)
{
	if (!OSCClient || !OSCClient->IsInitialized())
	{
		return false;
...
bool FDS100Processor::SendOSCBundle(const FOSCBundle& Bundle)
{
	if (!OSCClient || !OSCClient->IsInitialized())
	{
		return false;
...
bool FDS100Processor::SendQueuedMessages(const TArray<FOSCMessage>& Messages)
{
	if (!OSCClient || !OSCClient->IsInitialized())
	{
		return false;
...
void FDS100Processor::HandleReceivedOSCMessage(const FOSCMessage& Message)
{
	// Update stats
	{
...
void FDS100Processor::HandlePositionResponse(const FOSCMessage& Message)
{
